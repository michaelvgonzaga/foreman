...
- **Before any action that touches git data, filesystem state, or project metadata:** use `4orman-tools` — full subcommand map:
  ...
  | ledger outcome tracking — retrospective: did a past decision's prediction actually hold up? Applies to any entry, rps or jungian. `<matched\|diverged>` must be exactly one of those two words | `4orman-tools ledger record-outcome <id> <outcome> <matched\|diverged>` |
