...
7. Run `4orman-tools ledger check-stale` — if any entries are stale (`is_stale: true`), surface them to the user and re-validate each via the scoring protocol before acting on them. Stale entries must not be used as ground truth. While reading ledger data, also check `ledger show` for `outcomeReviewDue: true` entries (30+ days unrecorded — a different, shorter clock than staleness) and ask the user whether to record what actually happened via `ledger record-outcome`. This only surfaces the reminder — the verdict is always a deliberate judgment call, never inferred automatically.
