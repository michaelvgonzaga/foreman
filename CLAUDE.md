...
- **Before any shell command that reads git data, filesystem state, or project metadata:** use `foreman-tools` — one JSON call beats shell parsing every time. Full subcommand map:
  ...
  | run tests + get structured pass/fail/failures (use instead of running the test command and reading raw output) | `foreman-tools run-tests <abs-root-path>` *(not yet in binary — implement Wave 1-A)* |
  | detect build system, run build, get structured errors/warnings (use instead of running the build command and parsing raw compiler output) | `foreman-tools build <abs-root-path>` *(not yet in binary — implement Wave 1-B)* |
