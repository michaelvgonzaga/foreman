...
**Current:** `session-snapshot` done (anti-drift, v0.49.0). Next: Module 27 (Permissions/Sandbox) — `sandbox-check`, or Module 29 (Rollback/Recovery) — `rollback`.  
...
**Wave 4 so far:** framework cmds ✅, `registry` (M1) ✅, `capability-check` (M2) ✅, `route` (M3) ✅, `report` (M23) ✅, `metrics` (M26) ✅, `session-snapshot` (anti-drift) ✅ — current: v0.49.0  
**Repo:** `foreman-tools/` is a separate git repo — check its own CHANGELOG for version state (current: v0.49.0).
